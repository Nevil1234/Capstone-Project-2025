...
export default function Scorecalc() {
  ...
  const [discountPercentage, setDiscountPercentage] = React.useState<number | null>(null); // Store discount percentage
  ...
  const handleBarCodeScanned = async ({ data }: BarcodeScanningResult) => {
    try {
      ...
      const discount = calculateDiscountPercentage(score);
      setDiscountPercentage(discount);

      setIsEligibleForDonation(score >= 50);
      setIsEligibleForReselling(score >= 80);

      // Navigate to marketplace with discount percentage
      router.push({
  ...
  const calculateDiscountPercentage = (score: number): number => {
    if (score >= 80) return 10; // 10% discount for high-quality items
    if (score >= 50) return 20; // 20% discount for medium-quality items
    return 30; // 30% discount for low-quality items
  };
  ...
  return (
    <>
      {!isPermissionGranted ? (
        ...
        <>
          <Pressable onPress={() => setShowCamera(!showCamera)} style={styles.scannerContainer}>
            {showCamera ? (
              <>
                <CameraView
                  style={styles.camera}
              ...
              <View style={styles.qrPlaceholder}>
                <Feather name="camera" size={50} color={Colors.BLUE} />
